...
xmlSecKeysMngrPtr load_rsa_keys(char* key_file, char* key_file2);
...
main(int argc, char **argv) {
    ...
    mngr = load_rsa_keys(argv[2], argv[3]);
    if(mngr == NULL) {
        return(-1);
    }
 ...
 */
xmlSecKeysMngrPtr 
load_rsa_keys(char* key_file, char* key_file2) {
    xmlSecKeysMngrPtr mngr;
    xmlSecKeyPtr key;
    ...
    /* DAN: load X509 cert from the file */
    if (xmlSecCryptoAppKeyCertLoad(key, key_file2, xmlSecKeyDataFormatPem)) {
        fprintf(stderr,"Error: failed to load Cert from file \"%s\"\n", key_file);
        xmlSecKeysMngrDestroy(mngr);
        return(NULL);
    }
...
encrypt_file(xmlSecKeysMngrPtr mngr, const char* xml_file, const char* key_name) {
    ...
    /* add X509 data */
    if(xmlSecTmplKeyInfoAddX509Data(keyInfoNode2) == NULL) {
      fprintf(stderr, "Error: failed to add X509 data\n");
      goto done;
    }
